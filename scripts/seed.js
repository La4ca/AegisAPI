...
// Get MongoDB URI from command line argument or env
const mongoURI = process.argv[2] || process.env.MONGODB_URI;
if (!mongoURI) {
  console.error(
    "Please provide MongoDB URI as argument or set MONGODB_URI in .env",
  );
  process.exit(1);
}
...
const hospitals = [
  ...
  },
  {
    ...
    address: {
      street: "279 E Rodriguez Sr. Blvd",
      city: "Quezon City",
      state: "Metro Manila",
      zipCode: "1102",
      country: "Philippines",
...
const generateDoctors = (specs, hospitals) => [
  {
    ...
    specialization:
      specs.find((s) => s.name === "Cardiology")?.name || "Cardiology",
  ...
  },
  {
    ...
    specialization:
      specs.find((s) => s.name === "Neurology")?.name || "Neurology",
    hospital: hospitals[0]?._id,
    isActive: true,
  },
  {
    ...
    specialization:
      specs.find((s) => s.name === "Pediatrics")?.name || "Pediatrics",
  ...
  },
  {
    ...
    specialization:
      specs.find((s) => s.name === "Orthopedics")?.name || "Orthopedics",
    hospital: hospitals[1]?._id,
    isActive: true,
  },
  {
    ...
    specialization:
      specs.find((s) => s.name === "Dermatology")?.name || "Dermatology",
  ...
  },
  {
    ...
    specialization:
      specs.find((s) => s.name === "Ophthalmology")?.name || "Ophthalmology",
    hospital: hospitals[2]?._id,
    isActive: true,
  },
  {
  ...
  },
  {
    ...
    specialization:
      specs.find((s) => s.name === "Psychiatry")?.name || "Psychiatry",
    hospital: hospitals[3]?._id,
    isActive: true,
  },
];
...
const generatePatients = (doctors) => [
  {
    ...
    conditions: [
      ...
      },
      {
        name: "Diabetes",
        severity: "mild",
        diagnosedDate: new Date("2024-02-20"),
        isActive: true,
      },
    ],
    ...
    conditions: [
      {
        name: "Arthritis",
        severity: "severe",
        diagnosedDate: new Date("2023-11-05"),
        isActive: true,
      },
      {
        name: "Osteoporosis",
        severity: "moderate",
        diagnosedDate: new Date("2024-01-20"),
        isActive: true,
      },
    ],
    ...
    conditions: [
      {
        name: "Migraine",
        severity: "mild",
        diagnosedDate: new Date("2024-04-01"),
        isActive: true,
      },
    ],
    ...
    conditions: [
      {
        name: "Depression",
        severity: "moderate",
        diagnosedDate: new Date("2023-12-10"),
        isActive: true,
      },
      {
        name: "Anxiety",
    ...
    conditions: [
      {
        ...
        severity: "moderate",
        diagnosedDate: new Date("2024-02-28"),
        isActive: true,
      },
    ],
    ...
    conditions: [
      {
        name: "COPD",
        severity: "severe",
        diagnosedDate: new Date("2023-10-15"),
        isActive: true,
      },
    ],
...
const generateAppointments = (patients, doctors) => {
  ...
  patients.forEach((patient, idx) => {
    ...
    appointments.push({
      ...
      type: "in-person",
      status:
        idx % 4 === 0 ? "confirmed" : idx % 3 === 0 ? "completed" : "scheduled",
...
// ✅ Fixed referral generator (only 'normal' or 'urgent')
const generateReferrals = (patients, doctors) => {
  return patients.slice(0, 5).map((patient, idx) => ({
    ...
    priority: idx % 3 === 0 ? "urgent" : "normal", // only "normal" or "urgent"
